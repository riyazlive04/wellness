            ...
            <motion.div variants={fadeUp} className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
              ...
              {statusFilter === 'all' && !q && (
                <motion.button
                  ...
                  className="group flex min-h-[260px] flex-col items-center justify-center gap-2 rounded-3xl border border-dashed border-foreground/15 text-foreground/45 transition-colors hover:border-[hsl(var(--brand-blue))]/40 hover:bg-[hsl(var(--brand-blue))]/[0.05] hover:text-[hsl(var(--brand-blue))]"
                >
...
function ProgramCard({ t, index, expanded, onToggle, onPublish, publishing, onDelete, deletingId }: ProgramCardProps) {
  ...
  return (
    <motion.div
      ...
      onClick={onToggle}
      className={cn(
        'group relative flex cursor-pointer flex-col overflow-hidden rounded-3xl border bg-card shadow-sm transition-[transform,box-shadow,border-color] duration-200',
        expanded
          ? '-translate-y-0.5 border-[hsl(var(--brand-blue))]/30 shadow-[0_22px_46px_-24px_rgba(14,26,36,0.45)]'
          : 'border-foreground/[0.06] hover:-translate-y-1 hover:border-[hsl(var(--brand-blue))]/30 hover:shadow-[0_22px_46px_-24px_rgba(14,26,36,0.45)]',
      )}
    >
      {/* Accent spine */}
...
const KPI_TINT: Record<string, string> = {
  ...
  violet: 'bg-violet-100 text-violet-950 border-violet-200/60 dark:bg-violet-500/[0.12] dark:text-violet-50 dark:border-violet-500/20',
...
function KPIStat({
  tint = 'teal', icon: Icon, label, value, hint, suffix = '',
}: { tint?: string; icon: typeof Layers; label: string; value: number; hint: string; suffix?: string }) {
  ...
  return (
    <div className={cn('relative overflow-hidden rounded-2xl border p-5 shadow-sm', KPI_TINT[tint] ?? KPI_TINT.teal)}>
      ...
      <div className="mt-3 text-[30px] font-extrabold leading-none tracking-tight tabular-nums">{display}{suffix}</div>
