...
export default function ClientAppointments() {
  ...
  // A client booking is a request: 'pending' until the nutritionist confirms it.
  ...
  const past = appts.filter((a) => (
    ...
    || (a.status === 'scheduled' && new Date(a.scheduled_at).getTime() <= Date.now())
  ));
    ...
    <ClientLayout firstName={profileQ.data?.name?.split(' ')[0]}>
      <div className="mx-auto w-full max-w-6xl space-y-7 px-5 py-8 md:px-8 md:py-10">
        <motion.div variants={stagger(0.06, 0.05)} initial="initial" animate="animate" className="space-y-7">
          ...
          {/* Awaiting confirmation (client requests the nutritionist hasn't decided on yet) */}
      ...
      <div className="flex h-full items-start gap-3">
        <div className={cn(
          ...
          isPending ? 'bg-amber-500/15'
            : isUpcoming ? 'bg-gradient-to-br from-blue-500/20 to-cyan-500/15'
            : 'bg-foreground/[0.04]',
        )}>
        ...
        <div className="flex-1">
          ...
          {isPending && (
            <div className="mt-1.5 inline-flex items-center gap-1.5 text-[11px] text-amber-700 dark:text-amber-300">
              <Clock className="h-3 w-3" /> Waiting for your nutritionist to confirm
            </div>
          )}
          {appt.status === 'declined' && appt.cancel_reason && (
            <div className="mt-1.5 text-[11px] text-foreground/55">Reason: {appt.cancel_reason}</div>
          )}
          {appt.rescheduled_at && appt.status !== 'cancelled' && appt.status !== 'declined' && (
        ...
        {(isUpcoming || isPending) && onCancel && (
          ...
          >
            {cancelling ? <Loader2 className="h-3 w-3 animate-spin" /> : isPending ? 'Withdraw' : 'Cancel'}
          </button>
        )}
      </div>
    </Glass>
...
function BookingDialog({ onClose, onBooked }: { onClose: () => void; onBooked: () => void }) {
  ...
  const bookMut = useMutation({
    ...
    onSuccess: () => {
      toast.success('Request sent — your nutritionist will confirm.');
      onBooked();
    },
      ...
      <div className="pointer-events-none fixed inset-0 z-50 grid place-items-center p-4">
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-foreground/[0.06] p-4">
            <h2 className="text-base font-semibold">Request appointment</h2>
            <button
              type="button"
              onClick={onClose}
          ...
          <div className="space-y-4 p-5">
            <Field label="Type">
              <Select
                value={kind}
                onValueChange={(v) => setKind(v as Appointment['kind'])}
              >
                <SelectTrigger aria-label="Type" className={selectCls}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
