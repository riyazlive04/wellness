...
export function AppFooter({
  ...
  return (
    ...
    // when content is short. On DESKTOP it's `sticky bottom-0` (stays in view on
    // long pages; no bottom chrome to collide with). On MOBILE it flows normally
    // (`relative`) — otherwise a sticky footer floats over the scrolling content
    // and covers controls like the "Enable push" button, and it would fight the
    // fixed bottom-tab nav. Frosted, opaque bg so content passes cleanly under on
    // desktop. Parent must be a flex column (OwnerLayout / ClientLayout `<main>`).
    <footer className={cn('relative z-20 mt-auto flex-shrink-0 border-t border-foreground/[0.06] bg-canvas/85 px-6 py-3.5 backdrop-blur-xl md:sticky md:bottom-0', className)}>
