 ...
 *   - Sirah Digital logo embedded top-left
...
export async function generateFoodLibraryPdf(opts: GeneratePdfOptions): Promise<void> {
  const { foods, category, source, query, practiceName = 'SIRAH LIFE' } = opts;
  ...
  autoTable(doc, {
    ...
    didDrawPage: (data) => {
      ...
      if (logoDataUrl) {
        ...
        doc.text(
          'Generated by SIRAH LIFE · Nutrition Intelligence Engine',
          margin + 14, footerY,
        );
      } else {
        doc.text(
