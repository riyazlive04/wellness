...
export async function generateReportPdf(data: ReportData): Promise<{ pageCount: number; sizeKb: number; filename: string }> {
  const logo = await loadImageAsDataUrl('/sirah-logo.png').catch(() => null);
  ...
  ctx.y = drawHeader(ctx, heading, sub, data);
  ...
  drawFooters(ctx, data.workspaceName);

  const pageCount = doc.getNumberOfPages();
  const blob = doc.output('blob') as Blob;
  const sizeKb = Math.max(1, Math.round(blob.size / 1024));
  const filename = `NUSI-${slug(sub)}${data.target ? `-${slug(data.target.label)}` : ''}-${todayIso()}.pdf`;
  doc.save(filename);
...
function drawHeader(ctx: Ctx, heading: string, sub: string, data: ReportData): number {
  const { doc, pageW, margin } = ctx;
  const headerY = margin;
  let brandX = margin;
  if (ctx.logo) {
    doc.addImage(ctx.logo, 'PNG', margin, headerY - 6, 22, 22);
    brandX = margin + 30;
  }
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.setTextColor(120);
  doc.text(data.workspaceName.toUpperCase(), brandX, headerY + 8);
  doc.text(fmtDate(data.generatedAt), pageW - margin, headerY + 8, { align: 'right' });
  ...
  doc.setFontSize(20);
  doc.setTextColor(20);
  doc.text(heading, margin, headerY + 56);

  doc.setFontSize(10);
  doc.setTextColor(110);
  doc.text(sub, margin, headerY + 74);

  doc.setFontSize(8);
  doc.setTextColor(150);
  doc.text(data.periodLabel.toUpperCase(), margin, headerY + 90);
...
function drawFooters(ctx: Ctx, workspaceName: string): void {
  ...
  for (let p = 1; p <= count; p++) {
    ...
    doc.text(`Generated by NUSI · ${workspaceName}`, margin, footerY);
    doc.text(`Page ${p} of ${count}`, pageW - margin, footerY, { align: 'right' });
  }
}
