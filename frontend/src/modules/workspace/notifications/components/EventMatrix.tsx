...
export function EventMatrix({ events, channelLabels, enabledChannels, onToggle }: EventMatrixProps) {
  ...
  return (
    <Glass className="overflow-hidden">
      {/* Header row */}
      <div className="hidden grid-cols-[1.7fr_repeat(4,80px)] items-center gap-2 border-b border-foreground/[0.06] bg-foreground/[0.02] px-5 py-3 text-[10px] uppercase tracking-[0.18em] text-foreground/75 dark:text-foreground/55 md:grid">
        <div>Event</div>
        {CHANNEL_ORDER.map((c) => {
      ...
      {CATEGORY_ORDER.map((cat) => {
        ...
        return (
          <div key={cat}>
            <div className="border-b border-foreground/[0.04] bg-foreground/[0.01] px-5 py-2 text-[10px] uppercase tracking-[0.18em] text-foreground/75 dark:text-foreground/60">
              {CATEGORY_LABEL[cat]}
            </div>
              ...
              {rows.map((evt) => (
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-[11px] text-foreground/75 dark:text-foreground/60">{evt.description}</div>
                  </div>
