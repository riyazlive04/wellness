...
export function MemberRow({ member, onRoleChange, onRemove }: MemberRowProps) {
  ...
  return (
    <li className="relative border-b border-foreground/[0.04] last:border-0">
      <div className="grid grid-cols-[auto_1fr_140px_120px_120px_36px] items-center gap-4 px-5 py-3.5">
        ...
        <div className="relative">
          ...
          {member.status === 'active' && (
            <span className="absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full border border-canvas bg-emerald-400" />
          )}
        </div>
        ...
        <div className="relative">
          ...
          {menuOpen && !isOwner && (
            <>
              ...
              <div className="absolute right-0 top-9 z-20 w-56 overflow-hidden rounded-xl border border-foreground/10 bg-surface-2 shadow-[0_12px_32px_-12px_rgba(0,0,0,0.6)]">
                {member.status === 'invited' ? (
                  <>
                    <MenuItem
                      icon={Mail}
