  ...
  return (
    <Glass className="p-5 md:p-6">
      <div className="flex items-end justify-between gap-3">
        <div>
          <div className="text-[10px] uppercase tracking-[0.18em] text-foreground/75 dark:text-foreground/55">Total nutrition</div>
          <div className="mt-1 flex items-baseline gap-1.5">
            ...
            <span className="text-sm text-foreground/75 dark:text-foreground/60">kcal</span>
          </div>
        </div>
        <div className="text-xs text-foreground/75 dark:text-foreground/55">{items.length} items detected</div>
      </div>
      ...
      <div className="mt-5 h-2 w-full overflow-hidden rounded-full bg-foreground/[0.04]">
        <div className="flex h-full">
          ...
          <div className="h-full bg-teal-400"  style={{ width: `${carbsPct}%` }} />
      ...
      <div className="mt-4 grid grid-cols-2 gap-x-6 gap-y-3 sm:grid-cols-4">
        ...
        <Macro dot="bg-teal-400"  label="Carbs"   value={total.carbs}   unit="g" />
  ...
  return (
    <div>
      ...
      <div className="mt-1 text-lg font-medium tabular-nums text-foreground">
        {value} <span className="text-xs text-foreground/75 dark:text-foreground/60">{unit}</span>
      </div>
    </div>
  );
}
