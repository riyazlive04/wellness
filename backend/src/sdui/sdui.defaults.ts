/**
 * Server-driven UI — the stock layouts.
 *
 * These reproduce the screens as they shipped hardcoded, and they do three jobs
 * at once:
 *
 ...
 */
import type { UiScreen } from './sdui.types';
...
/**
 * Bump when anything in this file changes.
 *
 * It feeds the bundle's ETag. A workspace that has published nothing is served
 * these layouts, so without this a device would keep its cached copy of the
 * OLD defaults after a deploy changed them — the ETag is derived from published
 * revisions and the plan, neither of which moves when we edit this file.
 */
