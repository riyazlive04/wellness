/**
 * MIRROR of backend/src/sdui/sdui.types.ts — do not edit here alone.
 *
 * The backend file is the contract; this is the copy the app compiles against.
 * They are duplicated rather than shared because the two projects have no build
 * relationship: mobile ships as a signed binary on a release cadence of its own,
 * and a package boundary between them would make every schema tweak a two-repo
 * publish. The duplication is safe in one direction only — the app must tolerate
 * a server that knows MORE node types than this file does, which is exactly what
 * the unknown-node fallback in the renderer is for.
 */
